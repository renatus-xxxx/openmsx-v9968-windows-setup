...
if(Test-Path -LiteralPath $OutputDir){throw "The output folder must be a path that does not exist yet. Stopping to protect existing files.`n出力フォルダは未作成のパスを指定してください。既存ファイルを保護するため停止します。"}
...
$parts=@(for($i=0;$i -lt 32;$i++){
    ...
    if(!(Test-Path -LiteralPath $p -PathType Leaf) -or (Get-Item -LiteralPath $p).Length -ne 131072){throw "Missing or wrong size (131072 bytes required) / 不足またはサイズ不一致（131072 bytes が必要）: $name"}
    $p
})
$fonts=@(foreach($name in @('A1GTKFN.ROM','KANJI.ROM','fs-a1gt_kanjifont.rom')){
    $p=Join-Path $DumpDir $name
    if(Test-Path -LiteralPath $p -PathType Leaf){
        if((Get-Item -LiteralPath $p).Length -ne 262144){throw "Kanji ROM has the wrong size / 漢字 ROM のサイズ不一致: $name"}
        $sha=(Get-FileHash -LiteralPath $p -Algorithm SHA1).Hash
        if($spec.bios[1].sha1 -notcontains $sha){throw "Kanji ROM SHA-1 does not match a supported value / 漢字 ROM の SHA-1 が対応値と異なります: $name / $sha"}
        $p
    }
})
if(!$fonts.Count){throw 'A1GTKFN.ROM or KANJI.ROM (256 KiB) is required / A1GTKFN.ROM または KANJI.ROM（256 KiB）が必要です。'}
...
if($spec.bios[0].sha1 -notcontains $sha){throw "SHA-1 of the joined firmware does not match a supported value: $sha. Check the part order, transfer errors and the ROM revision of your machine. Nothing was written.`n結合結果の SHA-1 が対応値と異なります: $sha。順序・転送・実機の版を確認してください。出力せず停止します。"}
...
if($spec.bios[1].sha1 -notcontains $fontSha){throw 'The Kanji ROM changed while it was being read / 読取り中に漢字 ROM が変更されました。'}
...
Write-Host "Created / 作成完了: $OutputDir"
