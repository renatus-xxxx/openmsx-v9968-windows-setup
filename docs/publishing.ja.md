...
公開対象検査は `config/PUBLIC_FILES.txt` を使用します。ルート外のパス、禁止対象の導入済み環境・生成物、自作確認 ROM 以外の ROM、テキスト内の個人パス等の特定パターンも検査します。日英文書の冒頭の言語リンクと、BAT から entry.ps1 への参照を確認します。
...
今回の準備作業では commit・push・タグ作成・リリース作成を行いません。公開前の ZIP リンクは未提供の状態です。
