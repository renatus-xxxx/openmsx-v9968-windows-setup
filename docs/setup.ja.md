...
`setup-cbios-v9968.bat` を実行します。公式 openMSX 21.0 Windows x64 ZIP と V9968 派生版 **14215c7** (2026-09-24) を取得し、実行前に ZIP のサイズ・SHA-256 と実行ファイルの SHA-256 を検査します。C-BIOS **0.29** は公式 ZIP に同梱されています。通信量は合計約 18 MB、導入先は1環境あたり約 200 MB の余裕を見込み、失敗時の保管分は別途必要です。

C-BIOS_MSX2+_JP を基に、内蔵 V9968・日本語・60 Hz の構成を作成します。カートリッジイメージ用で、BASIC・ディスク環境は提供しません。


0.7.5 は更新した派生版を固定して使用します。既存環境を上書きせず、[更新・復旧手順](troubleshooting.ja.md#更新と復旧)に従って別フォルダへ導入してください。[変更点と検証結果](emulator-update-20260924.ja.md)も参照できます。
...
BAT は Windows PowerShell の標準モジュールを選択します。実行ポリシーの一時指定・環境変数はプロセス内だけで、グローバル PATH・レジストリを変更しません。管理者権限で依存ソフトを追加導入する処理もありません。進行状況とエラーは英日併記で表示します。各条件の意味は[トラブル対処](troubleshooting.ja.md)を参照してください。
...
掲載画像は以前の環境で撮影したものです。現在のプローブ結果は更新記録を参照してください。
