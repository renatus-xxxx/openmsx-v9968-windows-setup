...
既存のプローブとデモの ROM を変更せず、新しく固定した派生版で C-BIOS/Z80・FS-A1GT/R800 の動作を確認しました。R20 の自動選択は、以前の 0x31 から **0x11** になります。ROM の再ビルドや機種 XML の変更は不要でした。今回はローカルのソースツリーの更新で、プロジェクトのバージョンと公開済み ZIP は変更していません。
...
最初の2コマンドは `fsa1gt` でも実行します。比較スイートは両機種を検査します。旧 d884c4b の runtime には `-ExpectedR20 31` を指定してください。ホスト側の時間切れを避けるため、水面・残光を含む描画回帰検査は単独実行を推奨します。`test-water.ps1` の制限は引き続き60秒です。既存の水面・残光検査はデモのディレクトリ内にあります。今回は配布 ZIP を作成・更新していません。現在の公開対象一覧は150件、公開済み0.7.3のZIPは147件なので、旧ZIPへの `-ZipPath` 検査は不一致で停止します。この変更をリリースする際は別途バージョンを決め、公開済み0.7.3アセットを同名で置き換えずに配布物を作成してください。runtime や生のキャプチャ用フォルダには所有 BIOS が含まれるため、そのまま公開物へ入れないでください。
