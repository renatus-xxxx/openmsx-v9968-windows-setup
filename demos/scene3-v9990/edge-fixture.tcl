...
debug set_bp @_scene_ready@ {} {
 set f [open fixture.bin rb];set pattern [read $f];close $f
 if {@V9990@} {
  set even {};set odd {};binary scan $pattern cu* values
  set idx 0;foreach x $values {if {$idx&1} {append odd [binary format c $x]} else {append even [binary format c $x]};incr idx}
  debug write_block {Sunrise GFX9000 VRAM} 49152 $even
  debug write_block {Sunrise GFX9000 VRAM} 311296 $odd
 } else {debug write_block VRAM 98304 $pattern}
